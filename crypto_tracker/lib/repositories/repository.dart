...
class Repository {
  ...
  static Future<Preco> getExchange(CoinEnum deMoeda, CoinEnum paraMoeda) async {
    await dotenv.load(fileName: "keys.env");
    String apiKey = dotenv.env["API_KEY"]!;
    var url = Uri.https("rest.coinapi.io",
        "/v1/exchangerate/${deMoeda.name}/${paraMoeda.name}");
    http.Response response =
        await http.get(url, headers: {"X-CoinAPI-Key": apiKey});
    Map<String, dynamic> retorno = json.decode(response.body);
    Preco preco =
        Preco(deMoeda: deMoeda, paraMoeda: paraMoeda, valor: retorno["rate"]);
    return preco;
  }
}
